...
{
	char username[256];
	sprintf(username,"(%s)",user[clientId].username);
	...
	else if(strcmp((char *)dat+12,username) && '/'==(char)(dat+12+strlen(username))[0])
	{
		YsString msg((char *)dat+12+strlen(username)+1);
		auto argv=msg.Argv();
		if(0==argv.size())
		{
			return YSOK;
		...
		argv[0].Capitalize();
		auto extension = extensionCollection->GetExtension(argv[0]);
		if(nullptr==extension)
		{
			return YSOK;
		}
		extension->OnSlashCommand(*this,clientId,argv.Subset(1));
	}
	else
	{
		if(chatLogFp.Fp()!=NULL)
...
FsServerVariable::FsServerVariable(const char _name[],FsSimulation *_sim,FsNetConfig *_netcfg)
{
	...
	extensionCollection.reset(new FsNetExtensionCollection);
...
//	}
	...
	if(svr.choosingMode==0)
	{
		...
		{
		default:
			for(auto &extension : *svr.extensionCollection)
			{
				extension->OnNetConsoleCommand(svr,consCmd);
			}
			break;
...
void FsSimulation::RunServerModeOneStep(FsServerRunLoop &svrSta)
{
	...
	case FsServerRunLoop::SERVER_RUNSTATE_INITIALIZE3:
		{
			...
			for(auto &extension : *server.extensionCollection)
			{
				extension->StartSimulation(server);
			}
	...
	case FsServerRunLoop::SERVER_RUNSTATE_LOOP:
		{
			...
			for(auto &extension : *server.extensionCollection)
			{
				if(extension->IsRunning()==YSTRUE)
				{
					extension->OnInterval(server,passedTime);
				}
			}
	...
	case FsServerRunLoop::SERVER_RUNSTATE_TERMINATING:
		...
		for(auto &extension : *server.extensionCollection)
		{
			extension->EndSimulation(server);
		}
